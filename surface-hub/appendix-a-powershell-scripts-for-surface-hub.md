...
ms.locfileid: "10831087"
---
# <span data-ttu-id="8000c-104">PowerShell para Surface Hub</span><span class="sxs-lookup"><span data-stu-id="8000c-104">PowerShell for Surface Hub</span></span>

<span data-ttu-id="8000c-105">Scripts do PowerShell para ajudar a configurar e gerenciar o Microsoft Surface Hub.</span><span class="sxs-lookup"><span data-stu-id="8000c-105">PowerShell scripts to help set up and manage your Microsoft Surface Hub.</span></span>

-   [<span data-ttu-id="8000c-106">Scripts do PowerShell para administradores do Surface Hub</span><span class="sxs-lookup"><span data-stu-id="8000c-106">PowerShell scripts for Surface Hub admins</span></span>](#scripts-for-admins)
    -   [<span data-ttu-id="8000c-107">Criar uma conta local</span><span class="sxs-lookup"><span data-stu-id="8000c-107">Create an on-premises account</span></span>](#create-on-premises-ps-scripts)
    -   [<span data-ttu-id="8000c-108">Criar uma conta de dispositivo usando o Office 365</span><span class="sxs-lookup"><span data-stu-id="8000c-108">Create a device account using Office 365</span></span>](#create-os356-ps-scripts)
    -   [<span data-ttu-id="8000c-109">Script de verificação da conta</span><span class="sxs-lookup"><span data-stu-id="8000c-109">Account verification script</span></span>](#acct-verification-ps-scripts)
    -   [<span data-ttu-id="8000c-110">Habilitar o Skype for Business (EnableSfb.ps1)</span><span class="sxs-lookup"><span data-stu-id="8000c-110">Enable Skype for Business (EnableSfb.ps1)</span></span>](#enable-sfb-ps-scripts)
-   [<span data-ttu-id="8000c-111">Cmdlets úteis</span><span class="sxs-lookup"><span data-stu-id="8000c-111">Useful cmdlets</span></span>](#useful-cmdlets)
    -   [<span data-ttu-id="8000c-112">Criando uma política do Exchange ActiveSync compatível com o Surface Hub</span><span class="sxs-lookup"><span data-stu-id="8000c-112">Creating a Surface Hub-compatible Exchange ActiveSync policy</span></span>](#create-compatible-as-policy)
    -   [<span data-ttu-id="8000c-113">Permitindo IDs de dispositivo para o ActiveSync</span><span class="sxs-lookup"><span data-stu-id="8000c-113">Allowing device IDs for ActiveSync</span></span>](#whitelisting-device-ids-cmdlet)
    -   [<span data-ttu-id="8000c-114">Aceitando e recusando solicitações de reunião automaticamente</span><span class="sxs-lookup"><span data-stu-id="8000c-114">Auto-accepting and declining meeting requests</span></span>](#auto-accept-meetings-cmdlet)
    -   [<span data-ttu-id="8000c-115">Aceitando solicitações de reunião externas</span><span class="sxs-lookup"><span data-stu-id="8000c-115">Accepting external meeting requests</span></span>](#accept-ext-meetings-cmdlet)

## <span data-ttu-id="8000c-116">Pré-requisitos</span><span class="sxs-lookup"><span data-stu-id="8000c-116">Prerequisites</span></span>

<span data-ttu-id="8000c-117">Para executar esses scripts do PowerShell com êxito, você precisará instalar os pré-requisitos a seguir:</span><span class="sxs-lookup"><span data-stu-id="8000c-117">To successfully execute these PowerShell scripts, you will need to install the following prerequisites:</span></span>

- [<span data-ttu-id="8000c-118">Assistente de Conexão do Microsoft Online Services para Profissionais de TI RTW</span><span class="sxs-lookup"><span data-stu-id="8000c-118">Microsoft Online Services Sign-in Assistant for IT Professionals RTW</span></span>](https://www.microsoft.com/download/details.aspx?id=41950)
- [<span data-ttu-id="8000c-119">Microsoft Azure Active Directory Module for Windows PowerShell (versão de 64 bits)</span><span class="sxs-lookup"><span data-stu-id="8000c-119">Microsoft Azure Active Directory Module for Windows PowerShell (64-bit version)</span></span>](https://www.powershellgallery.com/packages/MSOnline/1.1.183.17)
- [<span data-ttu-id="8000c-120">Módulo do Windows PowerShell para Skype for Business Online</span><span class="sxs-lookup"><span data-stu-id="8000c-120">Windows PowerShell Module for Skype for Business Online</span></span>](https://www.microsoft.com/download/details.aspx?id=39366)

## <a href="" id="scripts-for-admins"></a><span data-ttu-id="8000c-121">Scripts do PowerShell para administradores do Surface Hub</span><span class="sxs-lookup"><span data-stu-id="8000c-121">PowerShell scripts for Surface Hub administrators</span></span>

<span data-ttu-id="8000c-122">O que os scripts fazem?</span><span class="sxs-lookup"><span data-stu-id="8000c-122">What do the scripts do?</span></span>

-   <span data-ttu-id="8000c-123">Criam contas de dispositivo para configurações usando uma única floresta pura local (somente Microsoft Exchange e Skype 2013 e posterior) ou online (Microsoft Office 365), que estão configuradas corretamente para o Surface Hub.</span><span class="sxs-lookup"><span data-stu-id="8000c-123">Create device accounts for setups using pure single-forest on-premises (Microsoft Exchange and Skype 2013 and later only) or online (Microsoft Office 365), that are configured correctly for your Surface Hub.</span></span>
-   <span data-ttu-id="8000c-124">Validam contas de dispositivo existentes para qualquer instalação (local ou on-line) para garantir a compatibilidade com o Surface Hub.</span><span class="sxs-lookup"><span data-stu-id="8000c-124">Validate existing device accounts for any setup (on-premises or online) to make sure they're compatible with Surface Hub.</span></span>
-   <span data-ttu-id="8000c-125">Fornecem um modelo de base para qualquer pessoa que desejar criar seus próprios scripts de criação ou validação de conta de dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8000c-125">Provide a base template for anyone wanting to create their own device account creation or validation scripts.</span></span>

<span data-ttu-id="8000c-126">Do que você precisa para executar os scripts?</span><span class="sxs-lookup"><span data-stu-id="8000c-126">What do you need in order to run the scripts?</span></span>

-   <span data-ttu-id="8000c-127">Acesso remoto do PowerShell ao domínio ou locatário da sua organização, servidores do Exchange e servidores do Skype for Business.</span><span class="sxs-lookup"><span data-stu-id="8000c-127">Remote PowerShell access to your organization's domain or tenant, Exchange servers, and Skype for Business servers.</span></span>
-   <span data-ttu-id="8000c-128">Credenciais de administrador do domínio ou locatário da sua organização, servidores do Exchange e servidores do Skype for Business.</span><span class="sxs-lookup"><span data-stu-id="8000c-128">Admin credentials for your organization's domain or tenant, Exchange servers, and Skype for Business servers.</span></span>

> [!NOTE]
> <span data-ttu-id="8000c-129">Se você estiver criando uma nova conta ou modificando uma conta existente, o script de validação verificará se sua conta de dispositivo está configurada corretamente.</span><span class="sxs-lookup"><span data-stu-id="8000c-129">Whether you’re creating a new account or modifying an already-existing account, the validation script will verify that your device account is configured correctly.</span></span> <span data-ttu-id="8000c-130">Você deve sempre executar o script de validação antes de adicionar uma conta de dispositivo ao Surface Hub.</span><span class="sxs-lookup"><span data-stu-id="8000c-130">You should always run the validation script before adding a device account to Surface Hub.</span></span>

## <span data-ttu-id="8000c-131">Executando os scripts</span><span class="sxs-lookup"><span data-stu-id="8000c-131">Running the scripts</span></span>

<span data-ttu-id="8000c-132">Os scripts de criação de conta irão:</span><span class="sxs-lookup"><span data-stu-id="8000c-132">The account creation scripts will:</span></span>

-   <span data-ttu-id="8000c-133">Solicitar as credenciais de administrador</span><span class="sxs-lookup"><span data-stu-id="8000c-133">Ask for administrator credentials</span></span>
-   <span data-ttu-id="8000c-134">Criar contas de dispositivo em seu domínio/locatário</span><span class="sxs-lookup"><span data-stu-id="8000c-134">Create device accounts in your domain/tenant</span></span>
-   <span data-ttu-id="8000c-135">Criar ou atribuir uma política do ActiveSync compatível com o Surface Hub para as contas de dispositivo</span><span class="sxs-lookup"><span data-stu-id="8000c-135">Create or assign a Surface Hub-compatible ActiveSync policy to the device account(s)</span></span>
-   <span data-ttu-id="8000c-136">Definir vários atributos para as contas criadas no Exchange e no Skype for Business.</span><span class="sxs-lookup"><span data-stu-id="8000c-136">Set various attributes for the created account(s) in Exchange and Skype for Business.</span></span>
-   <span data-ttu-id="8000c-137">Atribuir licenças e permissões às contas criadas</span><span class="sxs-lookup"><span data-stu-id="8000c-137">Assign licenses and permissions to the created account(s)</span></span>

<span data-ttu-id="8000c-138">Estes são os atributos definidos pelos scripts:</span><span class="sxs-lookup"><span data-stu-id="8000c-138">These are the attributes that are set by the scripts:</span></span>
...
<thead>
<tr class="header">
<th align="left"><span data-ttu-id="8000c-139">Cmdlet</span><span class="sxs-lookup"><span data-stu-id="8000c-139">Cmdlet</span></span></th>
<th align="left"><span data-ttu-id="8000c-140">Atributo</span><span class="sxs-lookup"><span data-stu-id="8000c-140">Attribute</span></span></th>
<th align="left"><span data-ttu-id="8000c-141">Valor</span><span class="sxs-lookup"><span data-stu-id="8000c-141">Value</span></span></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td align="left"><p><span data-ttu-id="8000c-142">Set-Mailbox</span><span class="sxs-lookup"><span data-stu-id="8000c-142">Set-Mailbox</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-143">RoomMailboxPassword</span><span class="sxs-lookup"><span data-stu-id="8000c-143">RoomMailboxPassword</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-144">Fornecido pelo usuário</span><span class="sxs-lookup"><span data-stu-id="8000c-144">User-provided</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-145">EnableRoomMailboxAccount</span><span class="sxs-lookup"><span data-stu-id="8000c-145">EnableRoomMailboxAccount</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-146">True</span><span class="sxs-lookup"><span data-stu-id="8000c-146">True</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-147">Tipo</span><span class="sxs-lookup"><span data-stu-id="8000c-147">Type</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-148">Sala</span><span class="sxs-lookup"><span data-stu-id="8000c-148">Room</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p><span data-ttu-id="8000c-149">Set-CalendarProcessing</span><span class="sxs-lookup"><span data-stu-id="8000c-149">Set-CalendarProcessing</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-150">AutomateProcessing</span><span class="sxs-lookup"><span data-stu-id="8000c-150">AutomateProcessing</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-151">AutoAccept</span><span class="sxs-lookup"><span data-stu-id="8000c-151">AutoAccept</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-152">RemovePrivateProperty</span><span class="sxs-lookup"><span data-stu-id="8000c-152">RemovePrivateProperty</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-153">False</span><span class="sxs-lookup"><span data-stu-id="8000c-153">False</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-154">DeleteSubject</span><span class="sxs-lookup"><span data-stu-id="8000c-154">DeleteSubject</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-155">False</span><span class="sxs-lookup"><span data-stu-id="8000c-155">False</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-156">DeleteComments</span><span class="sxs-lookup"><span data-stu-id="8000c-156">DeleteComments</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-157">False</span><span class="sxs-lookup"><span data-stu-id="8000c-157">False</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-158">AddOrganizerToSubject</span><span class="sxs-lookup"><span data-stu-id="8000c-158">AddOrganizerToSubject</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-159">False</span><span class="sxs-lookup"><span data-stu-id="8000c-159">False</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-160">AddAdditionalResponse</span><span class="sxs-lookup"><span data-stu-id="8000c-160">AddAdditionalResponse</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-161">True</span><span class="sxs-lookup"><span data-stu-id="8000c-161">True</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-162">AdditionalResponse</span><span class="sxs-lookup"><span data-stu-id="8000c-162">AdditionalResponse</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-163">&quot;Isso é uma sala do Surface Hub.&quot;</span><span class="sxs-lookup"><span data-stu-id="8000c-163">&quot;This is a Surface Hub room!&quot;</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p><span data-ttu-id="8000c-164">New-MobileDeviceMailboxPolicy</span><span class="sxs-lookup"><span data-stu-id="8000c-164">New-MobileDeviceMailboxPolicy</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-165">PasswordEnabled</span><span class="sxs-lookup"><span data-stu-id="8000c-165">PasswordEnabled</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-166">False</span><span class="sxs-lookup"><span data-stu-id="8000c-166">False</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-167">AllowNonProvisionableDevices</span><span class="sxs-lookup"><span data-stu-id="8000c-167">AllowNonProvisionableDevices</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-168">True</span><span class="sxs-lookup"><span data-stu-id="8000c-168">True</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p><span data-ttu-id="8000c-169">Enable-CSMeetingRoom</span><span class="sxs-lookup"><span data-stu-id="8000c-169">Enable-CSMeetingRoom</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-170">RegistrarPool</span><span class="sxs-lookup"><span data-stu-id="8000c-170">RegistrarPool</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-171">Fornecido pelo usuário</span><span class="sxs-lookup"><span data-stu-id="8000c-171">User-provided</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p></p></td>
<td align="left"><p><span data-ttu-id="8000c-172">SipAddress</span><span class="sxs-lookup"><span data-stu-id="8000c-172">SipAddress</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-173">Definir como o Nome Principal do Usuário (UPN) da conta de dispositivo</span><span class="sxs-lookup"><span data-stu-id="8000c-173">Set to the User Principal Name (UPN) of the device account</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p><span data-ttu-id="8000c-174">Set-MsolUserLicense (somente O365)</span><span class="sxs-lookup"><span data-stu-id="8000c-174">Set-MsolUserLicense (O365 only)</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-175">AddLicenses</span><span class="sxs-lookup"><span data-stu-id="8000c-175">AddLicenses</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-176">Fornecido pelo usuário</span><span class="sxs-lookup"><span data-stu-id="8000c-176">User-provided</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p><span data-ttu-id="8000c-177">Set-MsolUser (somente O365)</span><span class="sxs-lookup"><span data-stu-id="8000c-177">Set-MsolUser (O365 only)</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-178">PasswordNeverExpires</span><span class="sxs-lookup"><span data-stu-id="8000c-178">PasswordNeverExpires</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-179">True</span><span class="sxs-lookup"><span data-stu-id="8000c-179">True</span></span></p></td>
</tr>
<tr class="odd">
<td align="left"><p><span data-ttu-id="8000c-180">Set-AdUser (somente local)</span><span class="sxs-lookup"><span data-stu-id="8000c-180">Set-AdUser (On-prem only)</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-181">Habilitado</span><span class="sxs-lookup"><span data-stu-id="8000c-181">Enabled</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-182">True</span><span class="sxs-lookup"><span data-stu-id="8000c-182">True</span></span></p></td>
</tr>
<tr class="even">
<td align="left"><p><span data-ttu-id="8000c-183">Set-AdUser (somente local)</span><span class="sxs-lookup"><span data-stu-id="8000c-183">Set-AdUser (On-prem only)</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-184">PasswordNeverExpires</span><span class="sxs-lookup"><span data-stu-id="8000c-184">PasswordNeverExpires</span></span></p></td>
<td align="left"><p><span data-ttu-id="8000c-185">True</span><span class="sxs-lookup"><span data-stu-id="8000c-185">True</span></span></p></td>
</tr>
</tbody>
</table>

## <span data-ttu-id="8000c-186">Scripts de criação de conta</span><span class="sxs-lookup"><span data-stu-id="8000c-186">Account creation scripts</span></span>

<span data-ttu-id="8000c-187">Esses scripts criarão uma conta de dispositivo para você.</span><span class="sxs-lookup"><span data-stu-id="8000c-187">These scripts will create a device account for you.</span></span> <span data-ttu-id="8000c-188">Você pode usar o [Script de verificação da conta](#acct-verification-ps-scripts) para verificar se ele foi executado corretamente.</span><span class="sxs-lookup"><span data-stu-id="8000c-188">You can use the [Account verification script](#acct-verification-ps-scripts) to make sure they ran correctly.</span></span>

<span data-ttu-id="8000c-189">Os scripts de criação de conta não podem modificar uma conta existente, mas podem ser usados para ajudar você a compreender quais cmdlets precisam ser executados para configurar a conta existente corretamente.</span><span class="sxs-lookup"><span data-stu-id="8000c-189">The account creation scripts cannot modify an already existing account, but can be used to help you understand which cmdlets need to be run to configure the existing account correctly.</span></span>

### <a href="" id="create-on-premises-ps-scripts"></a><span data-ttu-id="8000c-190">Criar uma conta local</span><span class="sxs-lookup"><span data-stu-id="8000c-190">Create an on-premises account</span></span>

<span data-ttu-id="8000c-191">Cria uma conta, conforme descrito em [Implantação local](on-premises-deployment-surface-hub-device-accounts.md).</span><span class="sxs-lookup"><span data-stu-id="8000c-191">Creates an account as described in [On-premises deployment](on-premises-deployment-surface-hub-device-accounts.md).</span></span>
...
### <a href="" id="create-os356-ps-scripts"></a><span data-ttu-id="8000c-192">Criar uma conta de dispositivo usando o Office 365</span><span class="sxs-lookup"><span data-stu-id="8000c-192">Create a device account using Office 365</span></span>

<span data-ttu-id="8000c-193">Cria uma conta, conforme descrito em [Criar uma conta de dispositivo usando o Office 365](create-a-device-account-using-office-365.md)</span><span class="sxs-lookup"><span data-stu-id="8000c-193">Creates an account as described in [Create a device account using Office 365](create-a-device-account-using-office-365.md)</span></span>
...
## <a href="" id="acct-verification-ps-scripts"></a><span data-ttu-id="8000c-194">Script de verificação da conta</span><span class="sxs-lookup"><span data-stu-id="8000c-194">Account verification script</span></span>

<span data-ttu-id="8000c-195">Esse script validará a conta criada de dispositivo anteriormente em um Surface Hub, independentemente do método usado para criá-la.</span><span class="sxs-lookup"><span data-stu-id="8000c-195">This script will validate the previously-created device account on a Surface Hub, no matter which method was used to create it.</span></span> <span data-ttu-id="8000c-196">Esse script indica basicamente se a conta foi aprovada/reprovada.</span><span class="sxs-lookup"><span data-stu-id="8000c-196">This script is basically pass/fail.</span></span> <span data-ttu-id="8000c-197">Se ocorrer um erro de teste, será exibida uma mensagem de erro detalhada. Se todos os testes forem aprovados, o resultado final será um relatório de resumo.</span><span class="sxs-lookup"><span data-stu-id="8000c-197">If one of the test errors out, it will show a detailed error message, but if all tests pass, the end result will be a summary report.</span></span> <span data-ttu-id="8000c-198">Por exemplo, você pode ver:</span><span class="sxs-lookup"><span data-stu-id="8000c-198">For example, you might see:</span></span>
...
<span data-ttu-id="8000c-199">Detalhes de configurações específicas não serão mostrados.</span><span class="sxs-lookup"><span data-stu-id="8000c-199">Details of specific settings will not be shown.</span></span>
...
## <a href="" id="enable-sfb-ps-scripts"></a><span data-ttu-id="8000c-200">Habilitar o Skype for Business</span><span class="sxs-lookup"><span data-stu-id="8000c-200">Enable Skype for Business</span></span>

<span data-ttu-id="8000c-201">Esse script habilitará o Skype for Business em uma conta de dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8000c-201">This script will enable Skype for Business on a device account.</span></span> <span data-ttu-id="8000c-202">Use-o somente se o Skype for Business não foi habilitado anteriormente durante a criação da conta.</span><span class="sxs-lookup"><span data-stu-id="8000c-202">Use it only if Skype for Business wasn't previously enabled during account creation.</span></span>
...
## <span data-ttu-id="8000c-203">Cmdlets úteis</span><span class="sxs-lookup"><span data-stu-id="8000c-203">Useful cmdlets</span></span>

### <a href="" id="create-compatible-as-policy"></a><span data-ttu-id="8000c-204">Criando uma política do ActiveSync compatível com o Surface Hub</span><span class="sxs-lookup"><span data-stu-id="8000c-204">Creating a Surface Hub-compatible ActiveSync policy</span></span>

<span data-ttu-id="8000c-205">Para que o Surface Hub use os serviços do Exchange, uma conta de dispositivo configurada com uma política do ActiveSync compatível deve ser provisionada no dispositivo.</span><span class="sxs-lookup"><span data-stu-id="8000c-205">For Surface Hub to use Exchange services, a device account configured with a compatible ActiveSync policy must be provisioned on the device.</span></span> <span data-ttu-id="8000c-206">Essa política tem os seguintes requisitos:</span><span class="sxs-lookup"><span data-stu-id="8000c-206">This policy has the following requirements:</span></span>

``` syntax
PasswordEnabled == 0
```

<span data-ttu-id="8000c-207">Nos cmdlets a seguir, `$strPolicy` é o nome da política do ActiveSync, e `$strRoomUpn` é o UPN da conta do dispositivo que você deseja aplicar à política.</span><span class="sxs-lookup"><span data-stu-id="8000c-207">In the following cmdlets, `$strPolicy` is the name of the ActiveSync policy, and `$strRoomUpn` is the UPN of the device account you want to apply the policy to.</span></span>

<span data-ttu-id="8000c-208">Observe que, para executar os cmdlets, você precisa configurar uma sessão remota do PowerShell e:</span><span class="sxs-lookup"><span data-stu-id="8000c-208">Note that in order to run the cmdlets, you need to set up a remote PowerShell session and:</span></span>

-   <span data-ttu-id="8000c-209">Sua conta de administrador deve ser habilitada para PowerShell remoto.</span><span class="sxs-lookup"><span data-stu-id="8000c-209">Your admin account must be remote-PowerShell-enabled.</span></span> <span data-ttu-id="8000c-210">Isso permite que o administrador use os cmdlets do PowerShell que são necessários para o script.</span><span class="sxs-lookup"><span data-stu-id="8000c-210">This allows the admin to use the PowerShell cmdlets that are needed by the script.</span></span> <span data-ttu-id="8000c-211">(Essa permissão pode ser definida com `set-user $admin -RemotePowerShellEnabled $true`)</span><span class="sxs-lookup"><span data-stu-id="8000c-211">(This permission can be set using `set-user $admin -RemotePowerShellEnabled $true`)</span></span>
-   <span data-ttu-id="8000c-212">Sua conta de administrador deve ter a função "Redefinir senha" se você planeja executar scripts de criação.</span><span class="sxs-lookup"><span data-stu-id="8000c-212">Your admin account must have the "Reset Password" role if you plan to run the creation scripts.</span></span> <span data-ttu-id="8000c-213">Isso permite que o administrador altere a senha da conta, o que é necessário para o script.</span><span class="sxs-lookup"><span data-stu-id="8000c-213">This allows the admin to change the password of the account, which is needed for the script.</span></span> <span data-ttu-id="8000c-214">A função de redefinição de senha pode ser habilitada usando o Exchange Admin Center.</span><span class="sxs-lookup"><span data-stu-id="8000c-214">The Reset Password Role can be enabled using the Exchange Admin Center.</span></span>

<span data-ttu-id="8000c-215">Crie a política.</span><span class="sxs-lookup"><span data-stu-id="8000c-215">Create the policy.</span></span>
...
<span data-ttu-id="8000c-216">Para aplicar a política, a caixa de correio não pode ser um tipo de sala, pois ela precisa ser convertida em um usuário primeiro.</span><span class="sxs-lookup"><span data-stu-id="8000c-216">To apply the policy, the mailbox cannot be a room type, so it has to be converted into a user first.</span></span>
...
<span data-ttu-id="8000c-217">Agora a conta de dispositivo só precisa ser convertida de volta em um tipo de sala.</span><span class="sxs-lookup"><span data-stu-id="8000c-217">Now the device account just needs to be converted back into a room type.</span></span>
...
### <a href="" id="whitelisting-device-ids-cmdlet"></a><span data-ttu-id="8000c-218">Permitindo IDs de dispositivo para o ActiveSync</span><span class="sxs-lookup"><span data-stu-id="8000c-218">Allowing device IDs for ActiveSync</span></span>

<span data-ttu-id="8000c-219">Para permitir uma conta `$strRoomUpn`, execute o seguinte comando:</span><span class="sxs-lookup"><span data-stu-id="8000c-219">To allow an account `$strRoomUpn`, run the following command:</span></span>

```PowerShell
Set-CASMailbox –Identity $strRoomUpn –ActiveSyncAllowedDeviceIDs “<ID>”
```

<span data-ttu-id="8000c-220">Para localizar a ID do dispositivo, execute:</span><span class="sxs-lookup"><span data-stu-id="8000c-220">To find a device's ID, run:</span></span>

```PowerShell
Get-ActiveSyncDevice -Mailbox $strRoomUpn
```

<span data-ttu-id="8000c-221">Isso recupera as informações do dispositivo para cada dispositivo em que a conta foi provisionada, incluindo a propriedade `DeviceId`.</span><span class="sxs-lookup"><span data-stu-id="8000c-221">This retrieves device information for every device that the account has been provisioned on, Including the `DeviceId` property.</span></span>

### <a href="" id="auto-accept-meetings-cmdlet"></a><span data-ttu-id="8000c-222">Aceitando e recusando solicitações de reunião automaticamente</span><span class="sxs-lookup"><span data-stu-id="8000c-222">Auto-accepting and declining meeting requests</span></span>

<span data-ttu-id="8000c-223">Para que uma conta de dispositivo aceite ou recuse automaticamente solicitações de reunião com base na disponibilidade, o atributo **AutomateProcessing** deve ser definido como **AutoAccept**.</span><span class="sxs-lookup"><span data-stu-id="8000c-223">For a device account to automatically accept or decline meeting requests based on its availability, the **AutomateProcessing** attribute must be set to **AutoAccept**.</span></span> <span data-ttu-id="8000c-224">Isso é recomendado para evitar reuniões sobrepostas.</span><span class="sxs-lookup"><span data-stu-id="8000c-224">This is recommended as to prevent overlapping meetings.</span></span>

```PowerShell
Set-CalendarProcessing $strRoomUpn -AutomateProcessing AutoAccept
```

### <a href="" id="accept-ext-meetings-cmdlet"></a><span data-ttu-id="8000c-225">Aceitando solicitações de reunião externas</span><span class="sxs-lookup"><span data-stu-id="8000c-225">Accepting external meeting requests</span></span>

<span data-ttu-id="8000c-226">Para que uma conta de dispositivo aceite solicitações de reunião externas (uma solicitação de reunião de uma conta que não esteja no mesmo locatário/domínio), a conta de dispositivo deve ser definida para permitir o processamento de solicitações de reunião externas.</span><span class="sxs-lookup"><span data-stu-id="8000c-226">For a device account to accept external meeting requests (a meeting request from an account not in the same tenant/domain), the device account must be set to allow processing of external meeting requests.</span></span> <span data-ttu-id="8000c-227">Depois de definida, a conta de dispositivo aceitará ou recusará automaticamente solicitações de reunião de contas externas, bem como de contas locais.</span><span class="sxs-lookup"><span data-stu-id="8000c-227">Once set, the device account will automatically accept or decline meeting requests from external accounts as well as local accounts.</span></span>

> [!Note]
> <span data-ttu-id="8000c-228">Se o atributo **AutomateProcessing** não estiver definido como **AutoAccept**, a configuração não terá nenhum efeito.</span><span class="sxs-lookup"><span data-stu-id="8000c-228">If the **AutomateProcessing** attribute is not set to **AutoAccept**, then setting this will have no effect.</span></span>
