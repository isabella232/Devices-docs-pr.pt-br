# [Microsoft Surface Hub](index.yml)

# Surface Hub 2S

## Visão geral
### [Novidades na atualização do Windows 10 Team 2020](surface-hub-2020-update-whats-new.md)
### [Especificações técnicas do Surface Hub 2S 50 "](surface-hub-2s-techspecs.md) 
### [Especificações técnicas do Surface Hub 2S 85 "](surface-hub-2s-85.md)
### [Noções básicas do sistema operacional (Surface Hub)](differences-between-surface-hub-and-windows-10-enterprise.md)
### [Ajustar brilho, volume e entrada do Surface Hub 2S](surface-hub-2s-onscreen-display.md)
### [Usar o Microsoft Whiteboard no Surface Hub](https://support.office.com/article/use-microsoft-whiteboard-on-a-surface-hub-5c594985-129d-43f9-ace5-7dee96f7621d)

## Planeje
### [Guia de preparação do local do Surface Hub 2S](surface-hub-2s-site-readiness-guide.md) 
#### [Planejamento do local para o Surface Hub 2S](surface-hub-2s-site-planning.md)
#### [Início rápido do Surface Hub 2S](surface-hub-2s-quick-start.md)
#### [Instalar e montar o Surface Hub 2S 50"](surface-hub-2s-install-mount.md)
#### [Mover e manusear o Surface Hub 2S 85"](hub-move.md)
#### [Instalar e montar o Surface Hub 2S 85 "](surface-hub-2s-85-install-mount.md)
#### [Personalizar a montagem de parede do Surface Hub 2S 50"](surface-hub-2s-custom-install.md)
#### [Planilha de configuração](setup-worksheet-surface-hub.md)
#### [Visão geral do teclado numérico e das portas do Surface Hub 2S](surface-hub-2s-port-keypad-overview.md)
#### [Conectar dispositivos ao Surface Hub 2S](surface-hub-2s-connect.md)
### [Preparar o ambiente para o Microsoft Surface Hub](prepare-your-environment-for-surface-hub.md)


## Implantar
### [Adoção e treinamento do Surface Hub 2S](surface-hub-2s-adoption-kit.md)
### [Vídeos de adoção do Surface Hub 2S](surface-hub-2s-adoption-videos.md)
### [Primeira instalação do Surface Hub 2S](first-run-program-surface-hub.md)
### [Conectar dispositivos ao Surface Hub 2S](surface-hub-2s-connect.md)
### [Criar e testar uma conta de dispositivo](create-and-test-a-device-account-surface-hub.md)
### [Criar os pacotes de provisionamento para Surface Hub](provisioning-packages-for-surface-hub.md)
### [Instalar aplicativos no Microsoft Surface Hub](install-apps-on-surface-hub.md)
### [Criar contas locais do Surface Hub 2S com o PowerShell](https://docs.microsoft.com/microsoftteams/rooms/with-skype-for-business-server-2015)
### [Aplicativo Teams para Surface Hub](hub-teams-app.md)
### [Acessórios certificados do Teams para o Surface Hub 2S](surface-hub-2s-certifications.md)

## Gerenciar
### [Migrar para o Windows 10 Pro ou para o Enterprise no Surface Hub 2](surface-hub-2s-migrate-os.md)
### [Configurar o Windows 10 Pro ou Enterprise no Surface Hub 2](surface-hub-2-post-install.md)
### [Suplementos essenciais para Windows 10 Pro e Enterprise no Surface Hub 2](surface-hub-2-essential-add-ons.md)
### [Instale a Atualização do Windows 10 Team 2020](surface-hub-2020-update.md)  
### [Problemas conhecidos - Atualização do Windows 10 Team 2020](surface-hub-2020-team-update-known-issues.md)
### [Configurar contas administrativas não globais no Surface Hub](surface-hub-2s-nonglobal-admin.md)
### [Instalar e configurar o novo Microsoft Edge no Surface Hub](surface-hub-install-chromium-edge.md)        
### [Gerenciar o Surface Hub com um provedor MDM](manage-settings-with-mdm-for-surface-hub.md)
### [Gerenciamento local para configurações do Surface Hub 2S](local-management-surface-hub-settings.md)
### [Gerenciar rotação de senha de conta de dispositivo](password-management-for-surface-hub-device-accounts.md)
### [Gerenciar atualizações do Windows](manage-windows-updates-for-surface-hub.md)
### [Miracast em rede sem fio existente ou LAN](miracast-over-infrastructure.md)
### [Salvar sua chave do BitLocker](save-bitlocker-key-surface-hub.md)
### [Propriedades do Microsoft Exchange](exchange-properties-for-surface-hub-device-accounts.md)
### [Aplicando políticas do ActiveSync a contas de dispositivo](apply-activesync-policies-for-surface-hub-device-accounts.md)
### [Atualizar o firmware da caneta no Surface Hub 2S](surface-hub-2s-pen-firmware.md)


## Proteja
### [Visão geral da segurança do Surface Hub](surface-hub-security.md)
### [Proteger e gerenciar o Surface Hub 2S com SEMM e UEFI](surface-hub-2s-secure-with-uefi-semm.md)
### [Autenticação moderna no Surface Hub](surface-hub-modern-auth.md) 
### [Configurar a entrada sem senha no Surface Hub](surface-hub-2s-phone-authenticate.md) 
### [Como o Surface Hub resolve problemas de segurança do Wi-Fi Direct](surface-hub-wifi-direct.md)

## Solução de problemas
### [Recuperar e restaurar o Surface Hub 2S](surface-hub-2s-recover-reset.md)
### [Solução de problemas do Miracast no Surface Hub](miracast-troubleshooting.md)
### [Como empacotar e enviar seu Surface Hub 2S para serviço](surface-hub-2s-pack-components.md)
### [Histórico de alterações](surface-hub-2s-change-history.md)

# Surface Hub (v1)
## Visão geral
### [Novidades no Windows 10, versão 1703 para o Surface Hub](surfacehub-whats-new-1703.md)
### [Noções básicas do sistema operacional (Surface Hub)](differences-between-surface-hub-and-windows-10-enterprise.md)
### [Informações técnicas para 55 "Microsoft Surface Hub (v1)](surface-hub-technical-55.md)
### [Informações técnicas para 84 "Microsoft Surface Hub (v1)](surface-hub-technical-84.md)
### [Usar o Microsoft Whiteboard no Surface Hub](https://support.office.com/article/use-microsoft-whiteboard-on-a-surface-hub-5c594985-129d-43f9-ace5-7dee96f7621d)

## Planeje
### [Preparar o ambiente para o Microsoft Surface Hub](prepare-your-environment-for-surface-hub.md)
### [Guia de preparação do local do Surface Hub](surface-hub-site-readiness-guide.md) 
### [Instalar fisicamente o Microsoft Surface Hub](physically-install-your-surface-hub-device.md) 

## Implantar
### [Criar e testar uma conta de dispositivo](create-and-test-a-device-account-surface-hub.md)
#### [Implantação online](https://docs.microsoft.com/microsoftteams/rooms/with-office-365)

#### [Implantação local (floresta única)](https://docs.microsoft.com/microsoftteams/rooms/with-skype-for-business-server-2015)

#### [Implantação local (várias florestas)](https://docs.microsoft.com/skypeforbusiness/deploy/deploy-clients/multiple-forest-on-premises-deployments)
#### [Implantação híbrida](hybrid-deployment-surface-hub-device-accounts.md)
#### [Propriedades do Microsoft Exchange](exchange-properties-for-surface-hub-device-accounts.md)
#### [Aplicando políticas do ActiveSync a contas de dispositivo](apply-activesync-policies-for-surface-hub-device-accounts.md)
#### [Gerenciamento de senhas](password-management-for-surface-hub-device-accounts.md)
#### [Criar pacotes de provisionamento](provisioning-packages-for-surface-hub.md)
#### [Gerenciamento de grupo de administração](admin-group-management-for-surface-hub.md)
### [Configurar o Microsoft Surface Hub](set-up-your-surface-hub.md)
#### [Planilha de configuração](setup-worksheet-surface-hub.md)
#### [Programa de primeira execução](first-run-program-surface-hub.md)

## Gerencie
### [Gerenciar o Microsoft Surface Hub](manage-surface-hub.md) 
### [Instale e configure o novo Microsoft Edge no Surface Hub](surface-hub-install-chromium-edge.md)    
### [PowerShell para Surface Hub](appendix-a-powershell-scripts-for-surface-hub.md)
### [Gerenciamento remoto do Surface Hub](remote-surface-hub-management.md)
#### [Gerenciar configurações com um provedor MDM](manage-settings-with-mdm-for-surface-hub.md)
#### [Monitorar o Surface Hub](monitor-surface-hub.md)
#### [Atualizações do Windows](manage-windows-updates-for-surface-hub.md)
### [Gerenciar configurações do Surface Hub](manage-surface-hub-settings.md)
#### [Gerenciamento local para configurações do Surface Hub](local-management-surface-hub-settings.md)
#### [Acessibilidade](accessibility-surface-hub.md)
#### [Alterar a conta de dispositivo do Surface Hub](change-surface-hub-device-account.md)
#### [Redefinição do dispositivo](device-reset-surface-hub.md)
#### [Usar o nome de domínio totalmente qualificado com o Surface Hub](use-fully-qualified-domain-name-surface-hub.md)
#### [Gerenciamento de rede sem fio](wireless-network-management-for-surface-hub.md)
### [Implementar a Qualidade do Serviço no Surface Hub](surface-hub-qos.md)
### [Instalar aplicativos no Surface Hub](install-apps-on-surface-hub.md)
### [Configurar o menu Iniciar do Surface Hub](surface-hub-start-menu.md)
### [Configurar e usar o Microsoft Whiteboard](whiteboard-collaboration.md) 
### [Encerrar uma reunião do Surface Hub com a opção Encerrar a sessão](finishing-your-surface-hub-meeting.md)
### [Conectar outros dispositivos e exibir seu conteúdo com o Surface Hub](connect-and-display-with-surface-hub.md)
### [Miracast em rede sem fio existente ou LAN](miracast-over-infrastructure.md)
### [Habilitar autenticação com fio 802.1x](enable-8021x-wired-authentication.md)
### [Uso de um sistema de controle de sala](use-room-control-system-with-surface-hub.md)

## Proteja
### [Fazer logon no Surface Hub com o Microsoft Authenticator](surface-hub-authenticator-app.md)
### [Salve sua chave do BitLocker](save-bitlocker-key-surface-hub.md)
### [Autenticação moderna no Surface Hub](surface-hub-modern-auth.md) 
### [Configurar a entrada sem senha no Surface Hub](surface-hub-2s-phone-authenticate.md) 
### [Como o Surface Hub resolve problemas de segurança do Wi-Fi Direct](surface-hub-wifi-direct.md)

## Solução de problemas
### [Uso da Ferramenta de Recuperação do Surface Hub](surface-hub-recovery-tool.md)
### [Substituição da SSD do Surface Hub](surface-hub-ssd-replacement.md)
### [Principais soluções de suporte para o Surface Hub](support-solutions-surface-hub.md)
### [Solução de problemas do Microsoft Surface Hub](troubleshoot-surface-hub.md)
### [Histórico de atualizações do Surface Hub](surface-hub-update-history.md)
### [Problemas conhecidos: Windows 10 Team OS](surface-hub-2020-team-update-known-issues.md)
### [Como usar a recuperação da nuvem para o BitLocker em um Surface Hub](use-cloud-recovery-for-bitlocker-on-surfacehub.md)
### [Como usar a Ferramenta de Diagnóstico de Hardware do Surface Hub para testar uma conta de dispositivo](use-surface-hub-diagnostic-test-device-account.md)
### [Solução de problemas do Miracast no Surface Hub](miracast-troubleshooting.md)
### [Os canais 149 a 165 do Miracast do Surface Hub não têm suporte na Europa, no Japão e em Israel](surfacehub-miracast-not-supported-europe-japan-israel.md)
### [O que fazer se o aplicativo Conectar no Surface Hub sair inesperadamente](connect-app-in-surface-hub-unexpectedly-exits.md)
### [O Surface Hub pode instalar atualizações e reiniciar fora do horário de manutenção](surface-hub-installs-updates-and-restarts-outside-maintenance-hours.md)
### [Regulamento Geral sobre a Proteção de Dados e Surface Hub](general-data-privacy-regulation-and-surface-hub.md)
### [Downloads úteis para administradores do Surface Hub](surface-hub-downloads.md)
### [Histórico de alterações do Surface Hub](change-history-surface-hub.md)
