---
title: Guias de adoção e treinamento do Surface Hub 2S
description: A Microsoft desenvolveu materiais para download que você pode disponibilizar para os usuários auxiliarem na adoção do Surface Hub 2S.
keywords: separar os valores com vírgulas
ms.prod: surface-hub
ms.sitesec: library
author: greg-lindsay
ms.author: greglin
manager: laurawi
audience: Admin
ms.topic: article
ms.date: 11/04/2019
ms.localizationpriority: Medium
ms.openlocfilehash: 40a3c65c61950c0679e6af83e51d310b918d3be0
ms.sourcegitcommit: 109d1d7608ac4667564fa5369e8722e569b8ea36
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 06/27/2020
ms.locfileid: "10830545"
---
# Guias de adoção e treinamento do Surface Hub 2S

Seja você um negócio pequeno ou grande, um plano de adoção do Surface Hub é essencial para gerar os casos de uso corretos e ajudar os usuários a se familiarizar com o dispositivo. Confira estes guias para download projetados para ajudá-lo a fornecer treinamento em sua organização.

##  <a name="on-demand-training"></a>Treinamento sob demanda

- [Adoção e vídeos de treinamento do Surface Hub 2S](surface-hub-2s-adoption-videos.md)

##  <a name="adoption-toolkit"></a>Kit de ferramentas de adoção

- [Kit de ferramentas de adoção do Surface Hub](downloads/SurfaceHubAdoptionToolKit.pdf)

##  <a name="training-guides"></a>Guias de treinamento

- [Guia de treinamento – usuário final](downloads/TrainingGuide-SurfaceHub2S-EndUser.pdf)
- [Guia de treinamento – usuário avançado](downloads/TrainingGuide-SurfaceHub2S-PowerUser.pdf)
- [Guia de treinamento – Help Desk](downloads/TrainingGuide-SurfaceHub2S-HelpDesk.pdf)
- [Guia de treinamento – Microsoft Teams desktop](downloads/Guide-SurfaceHub2S-Teams.pptx)

[Baixar todos os guias de treinamento](https://download.microsoft.com/download/2/2/3/2234F70E-E65A-4790-93DF-F4C373A75B8E/SurfaceHub2S-TrainerGuides-July2019.zip)

##  <a name="end-user-guides"></a>Guias do usuário final

- [Guia para navegação no Surface Hub](downloads/Guide-SurfaceHub2S-Navigation.pptx)
- [Guia para o Office 365 no Surface Hub](downloads/Guide-SurfaceHub2S-Office365.pptx)
- [Guia para o Microsoft whiteboard no Surface Hub](downloads/Guide-SurfaceHub2S-Whiteboard.pptx)
- [Guia para o Microsoft Teams no Surface Hub](downloads/Guide-SurfaceHub2S-Teams.pptx)

[Baixar todos os guias do usuário final](https://download.microsoft.com/download/E/7/F/E7FC6611-BB55-43E1-AF36-7BD5CE6E0FE0/SurfaceHub2S-EndUserGuides-July2019.zip)

##  <a name="quick-reference-cards"></a>Cartões de referência rápida

- [Conectar seu computador](downloads/QRCConnectYourPC.pdf) 
- [Ingressar em uma reunião do teams](downloads/QRCJoinTeamsMeeting.pdf)
- [Gerenciar uma reunião do teams](downloads/QRCManageTeamsMeeting.pdf)
- [Noções básicas de navegação](downloads/QRCNavigationBasics.pdf)
- [Agendar uma reunião do teams](downloads/QRCScheduleTeamsMeeting.pdf)
- [Iniciar uma nova reunião do teams](downloads/QRCStartNewTeamsMeeting.pdf)
- [Compartilhar ou enviar um arquivo](downloads/QRCShareSendFile.pdf)
- [Entre para exibir reuniões e arquivos](downloads/QRCSignInToViewMeetingsFiles.pdf)
- [Quadro de comunicações avançado](downloads/QRCWhiteboardAdvanced.pdf)
- [Ferramentas do quadro de comunicações](downloads/QRCWhiteboardTools.pdf)

[Baixar todos os cartões de referência rápida](https://download.microsoft.com/download/E/7/F/E7FC6611-BB55-43E1-AF36-7BD5CE6E0FE0/SurfaceHub2S-EndUserGuides-July2019.zip)
